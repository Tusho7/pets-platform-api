...
export const deleteStreetPetById = async (req, res) => {
  const { petId } = req.params;

  try {
    const streetPet = await StreetPet.findByPk(petId);

    if (!streetPet) {
      return res.status(404).json({ message: "StreetPet not found" });
    }

    await streetPet.destroy();

    res.status(200).json({ message: "StreetPet deleted" });
  } catch (error) {
